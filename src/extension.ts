...
const HIGH_SCORE_KEY = 'neonJump.highScore';
...
export function activate(context: vscode.ExtensionContext): void {
  const disposable = vscode.commands.registerCommand('neonJump.start', () => {
    ...
    const highScore = context.globalState.get<number>(HIGH_SCORE_KEY, 0);
    ...
    panel.webview.onDidReceiveMessage(async (message) => {
      ...
      const currentHighScore = context.globalState.get<number>(HIGH_SCORE_KEY, 0);
...
function getWebviewHtml(
  ...
  const glUri = mediaUri('gl.js');
  const shadersUri = mediaUri('shaders.js');
  const rendererUri = mediaUri('renderer.js');
  const scriptUri = mediaUri('game.js');
  const styleUri = mediaUri('game.css');
...
<body>
  ...
  <script nonce="${nonce}" src="${glUri}"></script>
  <script nonce="${nonce}" src="${shadersUri}"></script>
